-- Criação de tabelas para a replicação do pacifico
use PROJETOPacific;
...
)ON [FG_PROJETOPacific_AccountManagement];
...
)ON [FG_PROJETOPacific_Customers];
...
-- Criação de tabelas para a replicação Europeia
use PROJETOEurope;
...
Ques_questionKey int identity(1,1) PRIMARY KEY,
Ques_Question nvarchar(100) NOT NULL UNIQUE
)ON [FG_PROJETOEurope_AccountManagement];
...
--exec SP_SPACEUSED
select * from PROJETOEurope.dbo.ProductColor;

insert into PROJETO.dbo.Currency(Cur_CurrencyKey, Cur_CurrencyAlternateKey, Cur_CurrencyName) values(900, '123', 'ACA');

set identity_insert PROJETO.dbo.ProductColor ON

insert into PROJETO.dbo.ProductColor(ProC_colorKey, ProC_color) values(900, 'Rainbow');
